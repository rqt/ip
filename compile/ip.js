...
const Y = z("aqt"), ca = async a => {
  const {data:b, type:c = "json", headers:f = {"User-Agent":`Mozilla/5.0 (Node.JS) ${X}`}, compress:d = !0, binary:e = !1, justHeaders:h = !1, method:l, timeout:n} = {}, g = M(!0), {hostname:r, protocol:u, port:v, path:w} = N(a), k = "https:" === u ? t : y, p = {hostname:r, port:v, path:w, headers:{...f}, timeout:n, method:l};
  if (b) {
    var q = c;
    var m = b;
    switch(q) {
  ...
  !d || "Accept-Encoding" in p.headers || (p.headers["Accept-Encoding"] = "gzip, deflate");
  const {body:x, headers:Z, byteLength:D, statusCode:aa, statusMessage:ba, h:E, f:F} = await W(k, p, {data:q, justHeaders:h, binary:e, a:g});
